const  brandsModel= require('../models/brand.Model');
...
function validateAddBrand(Brand) {
    const JoiSchema = Joi.object({
...
async function addBrand(req, res) {
    // const response = validateAddBrand(req.body);
    // if (response.error) {
    //     res.status(400).send({
    //       code: 400,
    ...
    //     });
    //   } else {
    var dataObj = new brandsModel(req.body);
    console.log(dataObj);
    dataObj.save(function (err, docs) {
        if (err) {
            ...
            res.status(201).send({
                code: 201,
                message: "Brand Save sucessfully",
                type: "success"
              });
        } else {
            res.status(400).send({ 
...
async function getCategorieById(req, res) {
    ...
    console.log(query);
    brandsModel.find(query).exec(function (err, docs) {
        if (err) {
            res.status(400).send({
                code: 400,
                message: "Invalid data",
            ...
            res.status(200).send({
                code: 200,
                message: "Brands Details Getting sucessfully",
                type: "success",
                data: docs
              });
        } else {
