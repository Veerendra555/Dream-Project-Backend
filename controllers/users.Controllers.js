...
async function addUser(req, res) {
    ...
    const response = validateAddUser(req.body)
    if (response.error) {
        res.status(200).send({
          code: 200,
...
async function getUsers(req, res) {
  console.log(req.query.role)
    ...
    query = {isActive : true};
    userModel.find(query).exec(function (err, docs) {
        if (err) {
            res.status(400).send({
                code: 400,
                message: "User Details Getting Error",
                type: "error",
                error: err
              });
        } else if (docs != null) {
            res.status(200).send({
                code: 200,
                message: "User Details Getting sucessfully",
                type: "success",
                data: docs
              });
        } else {
...
async function getUserById(req, res) {
    ...
    userModel.findById(query,{password:0}).exec(function (err, docs) {
        if (err) {
            res.status(400).send({
                code: 400,
                message: "Invalid data",
